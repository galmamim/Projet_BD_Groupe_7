...
public class Personnels {
	...
	public static void GestionPersonnels(Connection Conn) throws SQLException{
		...
		System.out.println("3-Affecter un Equipage � un Avion");
	...
	public static void AjouterPilote(Connection Conn)throws SQLException{
		...
		if(insert==0) {
			...
			System.out.println("Pilote Ajouter avec Succ�s!");
		}
	}
	public static void AfficherPilotes(Connection Conn)throws SQLException{
		...
		System.out.println("|Num�ro Pilote|Nombre d'Heures.Vol|Nom|Pr�nom|Num�ro|rue|Code Postale|Ville|Pays");
		System.out.println("\n");
		while(res.next())
		{      
	...
	public static void SupprimerPilote(Connection Conn) throws SQLException{
		...
		if(res2.next()) {
			...
			System.out.println("Veuillez s�lectionnez un Num�ro de Pilote: ");
			for(int c:l) {
			...
			while(!l.contains(nump)){
				System.out.println("Veuillez re-s�lectionner un Num�ro de Pilote: ");
				nump=Integer.parseInt(LectureClavier.lireChaine());
			}
		}else{
			System.out.println("Num�ro de Pilote Incorrecte !");
		}
		...
		if (delete==0) {
			...
			System.out.println("Pilote Supprimer avec Succ�s!");
		}
	}
	public static void AjouterQualife(Connection Conn) throws SQLException{
		...
		System.out.println("S�lectionnez le Pilote: ");
		Statement requete =conn.createStatement();
		ResultSet res2 =requete.executeQuery("SELECT numPilote FROM Pilotes ");
		int nump=0;
	...
	public static void AjouterHotesse(Connection Conn)throws SQLException{
		...
		System.out.println("Nombre d'Heures de Vol:");
		int nbvol = Integer.parseInt(LectureClavier.lireChaine());	
		System.out.println("Langues parl�es: ");
		...
		if(insert==0) {
			...
			System.out.println("Hotesse Ajouter avec Succ�s!");
		}
	}
	public static void AfficherHotesses(Connection Conn)throws SQLException{
		...
		ResultSetMetaData resultMeta = res.getMetaData();
		System.out.println("|Numero Hotesse|Nombre d'Heures.Vol|Nom|Prenom|Numero|rue|Code Postale|Ville|Pays");
		System.out.println("\n");
		while(res.next())
		{      
			for(int i = 1; i <=  resultMeta.getColumnCount(); i++)
	...
	public static void Affecter(Connection Conn)throws SQLException{
		...
		try {
			...
			System.out.println("Entrez un num�ro d'Hotesse");
			...
			ResultSetMetaData resultMeta = res.getMetaData();
			System.out.println("|Pilote Qualifi�|");
			while(res2.next())
			{      
				for(int i = 1; i <=  resultMeta.getColumnCount(); i++)
