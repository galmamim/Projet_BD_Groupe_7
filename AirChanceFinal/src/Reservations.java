...
public class Reservations {
	...
	public static void GestionReservation(Connection Conn) throws SQLException{
		...
		System.out.println("2-Afficher les R�servations d'un Client");
	...
	public static void AnnulerReservation(Connection Conn)throws SQLException{
		...
		try {
			System.out.println("Num�ro Client: ");
			...
			if(res2.next()) {
				...
				System.out.println("Veuillez s�lectionner le Num�ro de R�servation: ");
				for(int c:l) {
				...
				while(!l.contains(numR)){
					System.out.println("Veuillez re-s�lectionner un Num�ro de R�servation: ");
					numR=Integer.parseInt(LectureClavier.lireChaine());

				}
			}else{
				System.out.println("Vous n'avez Aucune R�servation !");
			}
			//System.out.println("Num�ro de R�servation");
	...
	public static void AjoutReservation(Connection Conn)throws SQLException{
		try {
			...
			System.out.println("Veuillez Entrez votre Num�ro: ");
			int numC=Integer.parseInt(LectureClavier.lireChaine());
	...
	public static void AfficherVol(String aerod,String aeroa) throws SQLException{
		try {
			...
			System.out.println("|Num�ro Vol|Num�ro d'Avion|A�roport D�part|A�roport Arriv�e|Horaire|Date d�part|");
			while(res2.next())
			{      
	...
	public static void AfficherPlaces(int numvc)throws SQLException{
		try {
			...
			ResultSetMetaData resultMeta = res2.getMetaData();
			System.out.println("|Num�ro de Place dans l'Avion|Classe|");
			while(res2.next())
			{      
				for(int i = 1; i <=  resultMeta.getColumnCount(); i++)
					System.out.print("|"+res2.getObject(i).toString());
	...
	public static void Transaction(int numR, int numClient,int numvc, String Place, int prixtot)throws SQLException{
		...
		try {	
			...
			System.out.println("Est ce que vous voulez valider la r�servation ?");
			String rep=LectureClavier.lireChaine();
			if(rep.equals("oui")) {
				conn.commit();
	...
	public static void AfficheReservation(Connection Conn)throws SQLException{
		try {
			...
			System.out.println("Num�ro du Client: ");
			int numc=Integer.parseInt(LectureClavier.lireChaine());
			
			System.out.println("Les R�servations trouv�es: ");
			...
			System.out.println("|Num�ro R�servation|Num�ro de Place|Num�ro de Vol |Num�ro du Client |Date de la r�servation|Prix Totale");
			while(res.next())
			{      
				for(int i = 1; i <=  resultMeta.getColumnCount(); i++)
