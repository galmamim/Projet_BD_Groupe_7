...
(
   ...
   constraint pk_avions primary key (numAvion),
	constraint fk_est foreign key (modele) references ModelesAvions (modele)on delete cascade
);
/*==============================================================*/
/* Table : VOLS                                                  */
/*==============================================================*/
...
(
   ...
   constraint PK_QUALIFIE primary key (MODELE, NUMPILOTE),
	...
	constraint FK_QUALIFIE2 foreign key (MODELE) references ModelesAvions (MODELE) on delete cascade
);
...
CREATE TRIGGER SUPPRESION_PLACES
AFTER DELETE
ON Avions
FOR EACH ROW
BEGIN
DELETE FROM PLACES
WHERE NUMAVION=:old.NUMAVION;
END;
/
